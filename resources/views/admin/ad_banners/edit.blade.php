...
@section('content')
    <div class="page-header">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div>
                <h1 class="page-title">Edit Ads Banner</h1>
                <p class="page-subtitle">Ubah banner iklan pada homepage</p>
            </div>
        ...
        <div class="card-body" style="padding: 30px;">
            <form action="{{ route('admin.ad-banners.update', $adBanner) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $adBanner->title) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input type="url" class="form-control @error('url_link') is-invalid @enderror" id="url_link" name="url_link" value="{{ old('url_link', $adBanner->url_link) }}" placeholder="https://example.com">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('target_routes') is-invalid @enderror" id="target_routes" name="target_routes" value="{{ old('target_routes', $adBanner->target_routes) }}" placeholder="Contoh: blog/detail/judul-artikel">
                ...
                <div class="form-group">
                    ...
                    @if($adBanner->image)
                        <div class="current-image" style="margin-bottom: 15px;">
                            <p style="margin-bottom: 5px; font-weight: 600; font-size: 0.9rem;">Gambar Saat Ini:</p>
                            <img src="{{ asset('storage/' . $adBanner->image) }}" alt="{{ $adBanner->title }}" style="max-width: 100%; height: auto; border-radius: 8px; border: 1px solid #e2e8f0; background: #f3f4f6;">
                        </div>
                    @endif

                    <div class="image-upload-wrapper">
                        <input type="file" class="form-control @error('image') is-invalid @enderror" id="image" name="image" accept="image/jpeg,image/png,image/webp" onchange="previewImage(this)">
                        <div class="image-preview" id="imagePreview" style="display: none; margin-top: 15px;">
                            <p style="margin-bottom: 5px; font-weight: 600; font-size: 0.9rem;">Gambar Baru:</p>
                            <img src="" alt="Preview" style="max-width: 100%; height: auto; border-radius: 8px; border: 1px solid #e2e8f0;">
                        </div>
                    </div>
                    <small class="text-muted">Format: JPG, PNG, WEBP. Maksimal: 2MB. Biarkan kosong jika tidak ingin mengubah gambar.</small>
                    @error('image')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group" style="margin-top: 20px;">
                    <div class="custom-control custom-switch">
                        <input type="checkbox" class="custom-control-input" id="is_active" name="is_active" value="1" {{ old('is_active', $adBanner->is_active) ? 'checked' : '' }}>
