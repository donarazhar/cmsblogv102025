...
@section('content')
    ...
    <div class="breadcrumb">
        ...
        <span>/</span>
        <span style="color: var(--dark); font-weight: 600;">Edit Tag</span>
    </div>

    <div class="form-card">
        <div class="form-header">
            <h2 class="form-title">Edit Tag</h2>
            <p class="form-subtitle">Perbarui informasi tag "{{ $tag->name }}"</p>
        </div>
            ...
            <div class="form-body">
                ...
                @if ($tag->posts_count > 0)
                    <div class="info-box">
                        ...
                        <div class="info-content">
                            <div class="info-title">Tag Sedang Digunakan</div>
                            <div class="info-text">
                                Tag ini saat ini digunakan oleh <strong>{{ $tag->posts_count }} post</strong>.
                                Perubahan akan mempengaruhi semua post yang menggunakan tag ini.
                            </div>
                        </div>
                    </div>
                @endif
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-input @error('name') error @enderror"
                        value="{{ old('name', $tag->name) }}" placeholder="Contoh: Tutorial, Berita, Teknologi" required
                        autofocus>
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="slug" name="slug" class="form-input @error('slug') error @enderror"
                        value="{{ old('slug', $tag->slug) }}" placeholder="slug-tag">
                    @error('slug')
                        <div class="form-error">
                            <i class="fas fa-exclamation-circle"></i>
                            <span>{{ $message }}</span>
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" class="form-input @error('description') error @enderror" rows="4"
                        placeholder="Deskripsi singkat tentang tag ini...">{{ old('description', $tag->description) }}</textarea>
                    @error('description')
                        <div class="form-error">
                            <i class="fas fa-exclamation-circle"></i>
                            <span>{{ $message }}</span>
                ...
                <div class="form-group">
                    ...
                    <div class="color-picker-wrapper">
                        ...
                        <div class="color-preview" id="colorPreview" style="background: {{ old('color', $tag->color) }};">
                            <span id="colorText">{{ strtoupper(old('color', $tag->color)) }}</span>
                        </div>
                    </div>
                    @error('color')
                        <div class="form-error">
                    ...
                    <div style="background: var(--light); border-radius: 8px; padding: 15px;">
                        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px; font-size: 0.9rem;">
                            <div>
                                ...
                                <strong
                                    style="display: block; margin-top: 4px;">{{ $tag->created_at->format('d M Y, H:i') }}</strong>
                            </div>
                            <div>
                                <span style="color: #6b7280;">Terakhir Diupdate:</span>
                                <strong
                                    style="display: block; margin-top: 4px;">{{ $tag->updated_at->format('d M Y, H:i') }}</strong>
                            </div>
                        </div>
                    </div>
                </div>
